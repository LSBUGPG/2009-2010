...
const int INVADER_START_Y = PLAY_FIELD_BOTTOM + INVADER_HEIGHT * 12;
...
void load_invaders(void)
{
	...
	for (i = 0; i < INVADER_TYPES; i++)
	{
		...
		invader_type_array[i].points = (i + 1) * 12;
	}

	load_audio(invader_hit, "hit.wav");

	int number = 0;
	int formation[] = { 0, 0, 1, 1, 2, 2, };
	...
	for (i = 0; i < BOMB_FRAMES; i++)
	{
		...
		bomb_image[i].dimension[0] = BOMB_WIDTH / 0.8;
		bomb_image[i].dimension[1] = BOMB_HEIGHT / 0.8;
	}
