...
const int INVADER_WIDTH = 46;
const int INVADER_HEIGHT = 46;
const int INVADER_START_X = PLAY_FIELD_LEFT + INVADER_WIDTH;
const int INVADER_START_Y = PLAY_FIELD_BOTTOM + INVADER_HEIGHT * 10;  // was 16
const int INVADER_OFFSCREEN = INVADER_HEIGHT * INVADER_ROWS * 3;
...
struct Commander: public Object
{
	...
	void update(void)
	{
	/*	int freq = 5 + (count * 50) / (INVADER_ROWS * INVADER_COLUMNS);  // whole section deals with invader beats seperation from actual movement rate
		count = 0;
		time++;
		if (time > freq)
		{
...
*/
		...
		{
			last_order = -1;
			if (descend)    // if descend has become true then make it false and change direction
			{
				descend = false;
				direction *= -1;
			}
			else if (edge)    // at an edge then use descend
			{
				edge = false;
				descend = true;
			}
	...
	bool order(int number, int move[])
	{
		...
		if (!win && order_given == -1 && number > last_order)
		{
			...
			{
				move[0] = 0;
				move[1] = -INVADER_HEIGHT / 2;  // move down
			}
			else
			{
				move[0] = direction * (INVADER_WIDTH / 8);
				move[1] = -INVADER_HEIGHT / 2 ; // move down again
			}
			return true;
		}
		else
...
struct Invader: public Object
{
	...
	void update(void)
	{
		...
		if (alive && commander->order(number, move))
		{
			...
			if (move[1] != 0)
			{
				if (position[1] <= PLAY_FIELD_BOTTOM - INVADER_OFFSCREEN) // are the invaders all off the bottom of the screen?
				{
					commander->landed();  // resets invaders 
		//			laser_invaded();   // causes player death should the invader hit the ground, left in for further development (could be used as a wave counter.
				}
			}
...
/*	for (i = 0; i < INVADER_BEATS; i++)
	{
		sprintf_s(filename, sizeof(filename), "walk%d.wav", i);     // Invader walking beats left in for testing purposes.
...
*/
	...
	int number = 0;
	int formation[] = { 0, 0, 1, 1, 2 };   // invader types and where they appear in the rows
...
void reset_invaders(void)
{
	...
	for (i = 0; i < INVADER_ROWS; i++)
	{
		...
		{
			Invader *invader = &invader_array[i][j];
			invader->position[0] = INVADER_START_X + j * INVADER_WIDTH * 2;  // width between invaders
			invader->position[1] = INVADER_START_Y + i * INVADER_HEIGHT * 4; // height between invaders
			invader->alive = true;
