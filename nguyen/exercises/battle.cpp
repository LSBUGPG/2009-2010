...
bool player_used_o_power = false;
...
void do_o_power(void)
{
    int bluff = roll(20) + get_modifier(player_strength) + player_level / 4;
    if (bluff >= 10 + monster_attack_modifier)
    {
        player_combat_advantage = 2;
        printf(
			"%s feels stronger and gains an advantage.\n",
			player_name,
			monster_type);
    }
    else
    {
        printf(
			"%s tries to us O power on %s but failed.\n",
			player_name,
			monster_type);
    }
    player_used_o_power = true;
}
...
void do_player_turn(void)
{
	...
	do
	{
		...
		if (!player_used_o_power)
		{
			printf("\tO - O power (once per combat)\n");
		}
        ...
        else if (!player_used_o_power && key == 'o')
		{
			do_o_power();
			done = true;
		}
