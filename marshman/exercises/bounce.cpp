...
HRESULT (WINAPI *DDrawCreate)( GUID FAR *lpGUID, LPDIRECTDRAW FAR *lplpDD, IUnknown FAR *pUnkOuter ) = NULL;

int main(int argc, char *argv[])
{
	HINSTANCE DDrawDLL = LoadLibrary(TEXT("DDRAW.DLL"));
	if ( DDrawDLL != NULL ) 
	{
		DDrawCreate = (HRESULT (__stdcall *)(GUID *,LPDIRECTDRAW *,IUnknown *))GetProcAddress(DDrawDLL, "DirectDrawCreate");
		if (DDrawCreate)
		{
			HRESULT result = DDrawCreate(NULL, &ddraw, NULL);
			if (result == S_OK)
			{
	...
	SDL_Surface *screen = SDL_SetVideoMode(SCREEN_WIDTH, SCREEN_HEIGHT, bpp, flags);
	if (screen == 0) 
	{
