...
describe_forest:
	printf(
		"You are in a mysterious forest. There is a mountain path to the south. \n"
		"Beside you is a small stream. The forest is\n"
		...
		"\n"
		"Will you ENTER the HUT or FOLLOW the STREAM? GO SOUTH to the mountain\n"
		"\n>");
...
else if (strstr(answer, "GO") && strstr(answer, "SOUTH"))
	{
	    goto go_south;
	}
	else
...
go_south:
    printf(
        "\n"
        "You head south to the mountain.\n"
        "\n");

    goto describe_mountain;
...
describe_mountain:
	printf(
		"You are at the foot of a mountain there is a dead warrior nearby.\n"
		"\n");

	if (have_sword)
	{
		printf(
			"You feel ashamed that you stole from the dead will you\n"
			"REPLACE SWORD, or GO back to the FOREST?\n"
			"\n>");

		fgets(answer, MAXIMUM_COMMAND_LENGTH, stdin);
		_strupr_s(answer, MAXIMUM_COMMAND_LENGTH);
		if (strstr(answer, "FOREST") && strstr(answer, "GO"))
		{
			goto leave_mountain;
		}
		else if (strstr(answer, "REPLACE") && strstr(answer, "SWORD"))
		{
			goto replace_sword;
		}
		else
		{
			goto unknown_mountain;
		}
	}
	else
	{
		printf(
			"There is a sword at the side of the warrior.\n"
			"\n"
			"Will you TAKE the SWORD or GO back to the FOREST?\n"
			"\n>");
		...
		else if (strstr(answer, "SWORD") && strstr(answer, "TAKE"))
		{
			goto take_sword;
		}
		else
...
replace_sword:
	printf(
		"\n"
		"You return the sword to its rightful owner.\n"
		"\n");
...
take_sword:
	printf(
		"\n"
		"You take the sword and head back to the forest.\n"
		"\n");
