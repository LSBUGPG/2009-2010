...
void do_player_nuke(void)
{
	player_hit_points = 0;
	player_used_nuke = true;
	printf("%s Pelvis Thrusts in the face of the %s as the nuke\n"
...
void do_player_turn(void)
{
	...
	do
	{
		...
		else if (!player_used_second_wind && key == 's')
		{
			...
			else if (!player_used_nuke && key == 'n')
		{
			do_player_nuke();
			done = true;
		}
