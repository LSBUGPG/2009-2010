...
const int BONUS_WIDTH = 32;
const int BONUS_HEIGHT = 16;
const int BONUS_SPEED = 1;
const int BONUS_FRAMES = 2;

Image bonus_image[BONUS_FRAMES];
...
struct Bonus: public Object
{
	...
	void update(void)
	{
		...
		{
			step++;
			position[0] += BONUS_SPEED;
			if (position[0] > PLAY_FIELD_RIGHT)
			{
				alive = false;
			}

			int size[] = { BONUS_WIDTH / 2, BONUS_HEIGHT / 2 };
			if (collide_laser(position, size))
			{
				alive = false;
			}
	...
	void draw(void)
	{
		...
		if (alive)
		{
			::draw(bonus_image[((step / 3) % 2)], fpos);
		}
	}
};
...
int load_invaders(Object *object_array[])
{
	...
	*object_array = &commander;
	...
	for (i = 0; i < INVADER_TYPES; i++)
	{
		...
		invader_type_array[i].points = points[i];
	}
	...
	for (i = 0; i < BONUS_FRAMES; i++)// load the bonus ship image and stuff..
	{
		sprintf_s(filename, sizeof(filename), "bonus%d.bmp", i);
		load_texture(bonus_image[i].texture, filename);
		bonus_image[i].dimension[0] = BONUS_WIDTH / 2;
		bonus_image[i].dimension[1] = BONUS_HEIGHT / 2;
	}
	
	bonus.position[0] = PLAY_FIELD_LEFT;
	bonus.position[1] = 100;
	bonus.alive = false;
	*object_array = &bonus;
...
bool collide_invaders(int position[], int size[])
{
	...
	if (!hit && bonus.alive)
	{
		if (position[0] - size[0] >= bonus.position[0] - BONUS_WIDTH  / 2 &&
			position[0] + size[0] <= bonus.position[0] + BONUS_WIDTH  / 2 &&
			position[1] - size[1] >= bonus.position[1] - BONUS_HEIGHT / 2 &&
			position[1] + size[1] <= bonus.position[1] + BONUS_HEIGHT / 2)
		{
			bonus.alive = false;
			add_points(200);
