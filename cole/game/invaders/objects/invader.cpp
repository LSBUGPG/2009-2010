...
const int INVADER_START_Y = PLAY_FIELD_BOTTOM + INVADER_HEIGHT * 50; // start hight
...
void reset_invaders(void)
{
	...
	for (i = 0; i < INVADER_ROWS; i++)
	{
		...
		{
			Invader *invader = &invader_array[i][j];
			invader->position[0] = INVADER_START_X + j * INVADER_WIDTH * 3;
			invader->position[1] = INVADER_START_Y + i * INVADER_HEIGHT * 6;
			invader->alive = true;
