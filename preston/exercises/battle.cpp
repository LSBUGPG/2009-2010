...
int player_initiative_modifier =	get_modifier(player_dexterity) + 
									player_level / 2;
const char *player_name = "Henrietta Shortwilly";
const char *player_weapon = "battle axe";
...
const char *monster_type = "Mutant Abomination";
const char *monster_weapon = "war glaive";
...
void do_player_berserk_attack(void)
{
	printf("%s makes a %s Berserk Attack on the %s.\n", player_name, player_weapon, monster_type);

	int dice = roll(16);
	bool critical = (dice == 16);
	int hit =   dice +
                get_modifier(player_strength) +
                player_weapon_proficiency +
                player_level / 2 +
	...
	if (critical || hit >= monster_armour_class)
	{
		...
		{
			damage = roll(8) + 4 + get_modifier(player_strength);
		}
		printf(
			"%s hit the %s for %d hit points of damage.\n",
			player_name,
