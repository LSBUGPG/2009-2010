...
const int INVADER_TYPES = 2;
const int INVADER_FRAMES = 2;
const int INVADER_WIDTH = 21;
const int INVADER_HEIGHT = 13;
...
const int BOMB_WIDTH = 3;
const int BOMB_HEIGHT = 7;
...
struct Commander: public Object
{
	...
	bool order(int number, int move[])
	{
		...
		if (!win && order_given == -1 && number > last_order)
		{
			...
			{
				move[0] = direction * (INVADER_WIDTH / 6);
				move[1] = 0;
			}
			return true;
		}
...
struct Invader: public Object
{
	...
	void update(void)
	{
		...
		if (alive && commander->order(number, move))
		{
			...
			if (move[0] != 0)
			{
				const GLfloat LEFT_MARGIN  = PLAY_FIELD_LEFT  - INVADER_WIDTH * 2;
				const GLfloat RIGHT_MARGIN = PLAY_FIELD_RIGHT + INVADER_WIDTH * 2;
...
void load_invaders(void)
{
	...
	for (i = 0; i < INVADER_TYPES; i++)
	{
		...
		{
			sprintf_s(filename, sizeof(filename), "plane%d.bmp", j);
			load_texture(invader_type_array[i].image[j].texture, filename);
			invader_type_array[i].image[j].dimension[0] = INVADER_WIDTH;
			invader_type_array[i].image[j].dimension[1] = INVADER_HEIGHT;
		}
		invader_type_array[i].points = (i + 1) * 10;
	}

	load_audio(invader_hit, "destruction.wav");
	...
	for (i = 0; i < BOMB_FRAMES; i++)
	{
		...
		bomb_image[i].dimension[0] = BOMB_WIDTH;
		bomb_image[i].dimension[1] = BOMB_HEIGHT;
	}

	load_audio(pommi_dropped, "pommi.wav");
...
void reset_invaders(void)
{
	...
	for (i = 0; i < INVADER_ROWS; i++)
	{
		...
		{
			Invader *invader = &invader_array[i][j];
			invader->position[0] = INVADER_START_X + i * INVADER_WIDTH * 2;
