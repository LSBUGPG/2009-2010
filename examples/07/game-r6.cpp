...
{
	GLfloat previous = position[0];

	position[0] += velocity[0] * t;
	position[1] += velocity[1] * t;

	GLfloat d = distance_to_table_top(*this);
	if (previous > 0.0f && position[0] < 0.0f && d < NET_HEIGHT)
	{
		add_point(left.score);
		serve(*this);
	}
	else if (previous < 0.0f && position[0] > 0.0f && d < NET_HEIGHT)
	{
	...
	{
		if (position[0] > TABLE_LEFT && position[0] < TABLE_RIGHT)
		{
			position[1] = TABLE_TOP + radius - d;
			velocity[1] *= -1.0f;
