...
int player_level = 1;
int player_armour_bonus = 7; // chain mail
int player_shield_bonus = 2; // broad shield
...
int player_initiative_modifier =	get_modifier(player_dexterity) + 
									player_level / 2;
const char *player_name = "crazy norsk warrior #256";
const char *player_weapon = "double-handed super sonic glaive";
...
const char *monster_type = "Grendal";
const char *monster_weapon = "monster claws";
...
void do_decide_initiative(void)
{
	...
	{
		printf("%s strikes first.\n", player_name);
		current = player_turn;
	}
	else
	{
		printf("The %s strikes first.\n", monster_type);
...
void do_player_melee(void)
{
	...
	if (critical || hit >= monster_armour_class)
	{
		...
		if (critical)
		{
			printf("%s's attack severely damages!\n", player_name);
...
void do_player_turn(void)
{
	...
	else
	{
		...
		{
			printf("%s calls for his dragon mother!.\n", monster_type);
		}
		current = monster_turn;
	}
}
...
int main(int argc, char *argv[])
{
	...
	do
	{
		...
		case player_wins:
			printf("%s defeats %s. for tyhe glory of Valhalla.\n", player_name, monster_type);
			current = end;
			break;

		case monster_wins:
			printf("%s has defeated %s. some one call beowulf!\n", monster_type, player_name);
			current = end;
			break;
