...
int player_total_hit_points = 27 + player_constitution;
...
int player_level = 1;
int player_armour_bonus = 8; // scale mail
int player_shield_bonus = 4; // heavy shield
...
int player_initiative_modifier =	get_modifier(player_dexterity) + 
									player_level / 2;
const char *player_name = "Ben";
const char *player_weapon = "axe";
...
const char *monster_type = "insane mother in law";
const char *monster_weapon = "hand bag";
...
void do_decide_initiative(void)
{
	...
	{
		printf("%s is quick to attack first.\n", player_name);
		current = player_turn;
	}
	else
	{
		printf("The %s seems fitter than she looks by striking first.\n", monster_type);
...
void do_monster_attack(void)
{
	...
	if (critical || hit >= player_armour_class())
	{
		...
		if (critical)
		{
			printf("The %s kicks where it hurts!\n", monster_type);
			damage = 10;
		}
		else
		{
...
{
	printf("%s is scared. Is that any way to treat family?\n", player_name);
...
void do_player_turn(void)
{
	...
	else
	{
		...
		{
			printf("The %s is on her last legs.\n", monster_type);
		}
		current = monster_turn;
	}
}
...
int main(int argc, char *argv[])
{
	...
	do
	{
		...
		case player_wins:
			printf("%s defeats the %s and can finally rule the family.\n", player_name, monster_type);
			current = end;
			break;

		case monster_wins:
			printf("The %s has defeated %s. Shame on you losing to a woman\n", monster_type, player_name);
			current = end;
			break;
