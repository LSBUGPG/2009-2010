...
const int INVADER_WIDTH = 22;
const int INVADER_HEIGHT = 12;
...
struct Bonus: public Object
{
	...
	void update(void)
	{
		if (alive)
		{
			if (position[0] < PLAY_FIELD_LEFT || position[0] > PLAY_FIELD_RIGHT)
			{
				direction *= -1;
			}
			position[0] += BONUS_SPEED * direction;
			position[1] -= BONUS_SPEED;
...
struct Commander: public Object
{
	...
	void update(void)
	{
		if (count == 10 && !bonus.alive)
		{
			bonus.alive = true;
			bonus.direction = 1;
			bonus.position[0] = 0;
			bonus.position[1] = PLAY_FIELD_TOP;
		}
...
void load_invaders(void)
{
	...
	{
		load_texture(bonus_image[i].texture, "nuke.bmp");
		bonus_image[i].dimension[0] = INVADER_WIDTH;
		bonus_image[i].dimension[1] = INVADER_HEIGHT;
	}
}
