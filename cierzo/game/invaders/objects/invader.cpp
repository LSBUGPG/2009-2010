...
struct Bomb: public Object
{
	...
	void draw(void)
	{
		GLfloat fpos[2] = { (GLfloat)position[0] + 1, (GLfloat)position[1] };//casting=converting int to floating point, so that you don't get warning when compiling.
		//put brackets around 'GLfloat' to say that we're converting the integer 'position' to a floating point, so that GL can draw it
		if (alive)
		{
...
void load_invaders(void)
{
	...
	for (i = 0; i < INVADER_TYPES; i++)
	{
		...
		{
			sprintf_s(filename, sizeof(filename), "ship%d.bmp", i, j);
	...
	{
		sprintf_s(filename, sizeof(filename), "goo%d.bmp", i);
