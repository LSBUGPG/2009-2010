...
int player_strength = 16 + player_level; // + human race bonus
...
bool player_used_over_power = false;
...
void do_player_melee(void)
{
	...
	if (critical || hit >= monster_armour_class)
	{
		...
		if (critical)
		{
			...
			damage = 8 + get_modifier(player_strength) + player_level * 2;
		}
		else
		{
			damage = roll(8) + get_modifier(player_strength) + player_level;
		}
		printf(
...
void do_player_over_power(void)
{
	player_level = get_modifier(player_level) + 999;
	player_used_over_power = true;
	printf("%s is now so OP he feels like a God.\n", player_name);
	do_player_defend();
}
...
void do_player_turn(void)
{
	...
	do
	{
		...
		if (!player_used_over_power)
		{
			printf("\tO - Over Power (once per combat)\n");
		}
		...
		else if (!player_used_over_power && key == 'o')
		{
			do_player_over_power();
			done = true;
		}
