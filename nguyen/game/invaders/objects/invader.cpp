...
const int INVADER_HEIGHT = 30;//changed height
const int INVADER_START_X = PLAY_FIELD_LEFT + INVADER_WIDTH;
const int INVADER_START_Y = PLAY_FIELD_BOTTOM + INVADER_HEIGHT * 5;
struct InvaderType
{
...
void load_invaders(void)
{
	...
	for (i = 0; i < INVADER_TYPES; i++)
	{
		for (j = 0; j < INVADER_FRAMES; j++)
		{
			...
			invader_type_array[i].image[j].dimension[0] = INVADER_WIDTH / 1.8f;//2.0
			invader_type_array[i].image[j].dimension[1] = INVADER_HEIGHT / 1.8f;//2.0
		}
