...
int player_initiative_modifier =	get_modifier(player_dexterity) + 
									player_level / 2;
const char *player_name = "George";
const char *player_weapon = "invisible sword"; 
bool player_used_visible = false;
...
const char *monster_type = "Evil Mime"; //random character here
const char *monster_weapon = "invisible baseball bat"; //random weapon here
...
void do_player_visible(void)
{
    int bluff = roll(20) + get_modifier(player_charisma) + player_level / 2;
    if (bluff >= 15 + monster_wisdom_modifier)
    {
        player_combat_advantage = 10;
        printf(
			"%s makes the %s visible which shocks the %s and gives\n" 
			"you an advantage.\n",
			player_name,
			player_weapon,
			monster_type);
    ...
    {
        printf(
			"%s tries to make the %s visible, but failed.\n",
			player_name,
			player_weapon);
    }
    player_used_visible = true;
}
...
void do_player_turn(void)
{
	...
	do
	{
		...
		printf("\tD - Defend\n");
		if (!player_used_visible)
		{
		    printf("\tV - Visibility (once per combat)\n");
		}
		printf(">");
		...
		else if (!player_used_visible && key == 'v')
		{
			do_player_visible();
			done = true;
		}
