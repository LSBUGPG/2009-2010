...
int player_initiative_modifier =	get_modifier(player_dexterity) + 
									player_level / 2;
const char *player_name = "Fagballs";
const char *player_weapon = "piece of string";
...
const char *monster_type = "Heather Mills";
const char *monster_weapon = "Wooden Leg";
...
void do_decide_initiative(void)
{
	...
	{
		printf("%s decides to beat her to it.\n", player_name);
		current = player_turn;
	}
	else
	{
		printf("%s beat you to it.\n", monster_type);
...
void do_monster_attack(void)
{
	...
	if (critical || hit >= player_armour_class())
	{
		...
		if (critical)
		{
			printf("%s kicked you in the ass!\n", monster_type);
			damage = 10;
		}
		else
		{
		...
		printf(
			"%s Bludgeoned %s for %d hit points of damage.\n",
			monster_type,
			player_name,
			damage);
	...
	{
	    printf("%s missed and hit paul mccartney's lawyer.\n", monster_type);
	}
...
void do_player_melee(void)
{
	...
	if (critical || hit >= monster_armour_class)
	{
		...
		printf(
			"%s struck %s for %d hit points of damage.\n",
			player_name,
			monster_type,
			damage);
...
void do_player_feint(void)
{
    ...
    if (bluff >= 10 + monster_wisdom_modifier)
    {
        ...
        printf(
			"%s compliments %s and gains an advantage.\n",
			player_name,
			monster_type);
    }
    else
    {
        printf(
			"%s said he prefferes Naomi Campbell to %s and pissed her off.\n",
			player_name,
			monster_type);
    }
...
void do_player_second_wind(void)
{
	...
	printf("%s realised he had to do something.\n", player_name);
	do_player_defend();
}
