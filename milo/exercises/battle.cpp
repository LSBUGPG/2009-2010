...
int player_constitution = 14;
int player_total_hit_points = 15 + player_constitution;
int player_hit_points = player_total_hit_points;
int player_strength = 16 + 3; // + Mage Race Bounus
int player_dexterity = 15;
int player_charisma = 15;
int player_level = 1;
int player_armour_bonus = 7; // Leather shards
int player_shield_bonus = 2; // Thome book of defense
int player_defence_bonus = 0; // bonus when defending
int player_weapon_proficiency = 5;
int player_combat_advantage = 0;
int player_initiative_modifier =	get_modifier(player_strength) + 
									player_level / 2;


const char *player_name = "Sunty";
const char *player_weapon = "The Iron Jang Bong War Staff";
bool player_used_second_wind = false;
bool player_used_feint = false;
bool player_used_action_point = false;
int player_initiative;

int monster_total_hit_points = 40;
int monster_hit_points = monster_total_hit_points;
int monster_dexterity = 25;
int monster_wisdom_modifier = 1;
int monster_initiative_modifier = 5;
int monster_attack_modifier = 6;
int monster_armour_class = 17;
const char *monster_type = "Ratise ice minion";
const char *monster_weapon = "Ice element";
...
void do_monster_attack(void)
{
	...
	if (critical || hit >= player_armour_class())
	{
		...
		if (critical)
		{
			printf("The %s made a critical hit! What are the chances of that happening!?\n", monster_type);
			damage = 10;
		}
		else
		{
...
void do_player_feint(void)
{
    ...
    if (bluff >= 10 + monster_wisdom_modifier)
    {
        ...
        printf(
			"%s fakes out the %s and gains the upper-hand.\n",
			player_name,
			monster_type);
    }
    else
    {
        printf(
			"%s tries to fake out the %s but failed miserably.\n",
			player_name,
			monster_type);
    }
...
void do_player_second_wind(void)
{
	...
	printf("%s feels a fresh surge of energy coming deep within his soul.\n", player_name);
	do_player_defend();
}
...
void do_player_turn(void)
{
	...
	do
	{
		...
		{
			printf("Not available, think it through!.\n");
		}
...
int main(int argc, char *argv[])
{
	...
	do
	{
		...
		case player_wins:
			printf("%s defeats the %s. Thank heavens\n", player_name, monster_type);
			current = end;
			break;

		case monster_wins:
			printf("The %s has defeated %s. Thank hell!\n", monster_type, player_name);
			current = end;
			break;
