...
struct Laser: public Object
{
	...
	void update(void)
	{
		if (alive)
		{
            ...
            if (move_up && position[1] < PLAY_FIELD_TOP - LASER_HEIGHT)//new
			{
				position[1]++;
			}

            if (move_down && position[1] > PLAY_FIELD_BOTTOM + LASER_HEIGHT)//new
			{
				position[1]--;
			}
...
{
	char filename[] = "ship0.bmp";//laser?
	for (int i = 0; i < LASER_FRAMES; i++)
	{
		sprintf_s(filename, sizeof(filename), "ship%d.bmp", i);
	...
	load_texture(shot.image.texture,"car0.bmp");//com0/ shot.bmp
