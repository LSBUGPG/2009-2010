...
const int SHOT_WIDTH = 1;
const int SHOT_HEIGHT = 5;
...
const int LASER_WIDTH = 22;
const int LASER_HEIGHT = 19;
...
struct Laser: public Object
{
	...
	void update(void)
	{
		if (alive)
		{
			//play_sound(enemy); tried a background sound but it covered everything else.
...
{
	char filename[] = "tank0.bmp";
	for (int i = 0; i < LASER_FRAMES; i++)
	{
		sprintf_s(filename, sizeof(filename), "tank%d.bmp", i);
		load_texture(laser.image[i].texture, filename);
		laser.image[i].dimension[0] = LASER_WIDTH;
		laser.image[i].dimension[1] = LASER_HEIGHT;
	}

	load_audio(laser.explode, "explotion.wav");
	//load_audio(laser.enemy, "airplane.wav");

	reset_laser();

	load_audio(shot.sound, "shotsound.wav");
	load_texture(shot.image.texture, "bullet.bmp");
	shot.image.dimension[0] = SHOT_WIDTH;
	shot.image.dimension[1] = SHOT_HEIGHT;
	shot.alive = false;
