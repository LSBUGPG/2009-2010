...
int player_strength = 30 + 2; // + human race bonus
int player_dexterity = 14;
int player_charisma = 6;
int player_level = 1;
int player_armour_bonus = 7; // scale mail
int player_shield_bonus = 6; // heavy shield
...
int player_initiative_modifier =	get_modifier(player_dexterity) + 
									player_level / 2;
const char *player_name = "Mad Sun";
const char *player_weapon = "Sun Ray";
bool player_used_second_wind = false;
bool player_used_frighten = false;
bool player_used_action_point = false;
int player_initiative;

int monster_total_hit_points = 55;
int monster_hit_points = monster_total_hit_points;
int monster_dexterity = 10;
int monster_wisdom_modifier = 1;
int monster_initiative_modifier = 2;
int monster_attack_modifier = 7;
int monster_armour_class = 11;
const char *monster_type = "Father Night";
const char *monster_weapon = "Evil Tome";
...
void do_decide_initiative(void)
{
	...
	{
		printf("%s gets in quick!\n", player_name);
		current = player_turn;
	}
	else
	{
		printf("The %s appears from nowhere!\n", monster_type);
...
{
	printf(
		"The %s reads from the %s and corrupts %s.\n", 
		monster_type, 
		monster_weapon, 
		player_name);
	...
	else
	{
		...
		{
			printf("%s lights are growing dim....\n", player_name);
		}
		current = player_turn;
	}
}
...
void do_player_frighten(void)
{
    ...
    if (bluff >= 10 + monster_wisdom_modifier)
    {
        ...
        printf(
			"%s scares the %s and gains an advantage.\n",
			player_name,
			monster_type);
    }
    else
...
void do_player_turn(void)
{
	...
	do
	{
		...
		printf("\tD - Defend\n");
		if (!player_used_frighten)
		{
		    printf("\tF - frighten (once per combat)\n");
		}
		...
		else if (!player_used_frighten && key == 'f')
		{
			do_player_frighten();
			done = true;
		}
...
int main(int argc, char *argv[])
{
	...
	do
	{
		...
		case player_wins:
			printf("%s conquers the %s.\n", player_name, monster_type);
			current = end;
			break;

		case monster_wins:
			printf("The %s spreads his darkness and turns %s to dust.\n", monster_type, player_name);
			current = end;
			break;
