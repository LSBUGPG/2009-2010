...
SoundKey sound_key[] =
{
	...
	SoundKey("buttona.bmp",
			 "SongA.wav",
			 Vertex(-300.0f, 150.0f),
			 SDLK_a),

	SoundKey("buttonb.bmp",
			 "SongB.wav",
			 Vertex( -150.0f, 150.0f),
			 SDLK_b),

	SoundKey("buttonc.bmp",
			 "SongC.wav",
			 Vertex( 0.0f, 150.0f),
			 SDLK_c),

	SoundKey("buttond.bmp",
			 "SongD.wav",
			 Vertex( 150.0f, 150.0f),
			 SDLK_d),

	SoundKey("buttone.bmp",
			 "SongE.wav",
			 Vertex( 300.0f, 150.0f),
			 SDLK_e),

	SoundKey("buttonf.bmp",
			 "SongF.wav",
			 Vertex( -300.0f, 00.0f),
			 SDLK_f),
			 
	SoundKey("buttong.bmp",
			 "SongG.wav",
			 Vertex( -150.0f, 00.0f),
			 SDLK_g),

	SoundKey("buttonh.bmp",
			 "SongH.wav",
			 Vertex( 0.0f, 00.0f),
			 SDLK_h),

			 
	SoundKey("buttoni.bmp",
			 "SongI.wav",
			 Vertex( 150.0f, 00.0f),
			 SDLK_i),
			 			 
	SoundKey("buttonj.bmp",
			 "SongJ.wav",
			 Vertex( 300.0f, 00.0f),
			 SDLK_j),
};
	...
	else
	{
		for (int i = 0; i < SOUNDS; i++)
		{
			if (key->sym == sound_key[i].keycode)
			{
				...
				{
					Mix_PlayMusic(sound_key[i].stream, 1);
//					Mix_HaltMusic();
...
void load(SoundKey &key)
{
	...
	key.stream = Mix_LoadMUS(key.audio);
	if (!key.stream)
	{
...
int main(int argc, char *argv[])
{
	...
	while (true) 
	{
		process_events();
		start_render();
		draw_bitmap(background, Vertex(0.0f, 0.0f), 400.0f);
		for (i = 0; i < SOUNDS; i++)
		{
			draw(sound_key[i]);
		}
