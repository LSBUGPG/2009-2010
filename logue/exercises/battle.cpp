...
int player_initiative_modifier =	get_modifier(player_dexterity) + 
									player_level / 2;
const char *player_name = "a little puppy";
const char *player_weapon = "flaming scythe of Hades";
...
const char *monster_weapon = "napalm";
...
void do_player_crotch_kick(void)
{
	printf("%s kicks %s in the mummy-daddy button.\n", player_name, monster_type);
	...
	if (critical || hit >= monster_armour_class)
	{
		...
		if (critical)
		{
			printf("Critical hit - %s made sure %s won't have any offspring\n", player_name, monster_type);
			damage = 10 + get_modifier(player_strength);
		}
		else
		{
			damage = roll(9) + get_modifier(player_strength);
		}
		printf(
			"%s hit %s for %d hit points of damage\n"
            "and gave %s a limp.\n",
	...
	{
	    printf("%s missed and fell on %s's arse.\n", player_name, player_name);
	}
}
...
void do_player_second_wind(void)
{
	...
	printf("%s gobbles some pedigree chum and is buzzing.\n", player_name);
	do_player_defend();
}
...
int main(int argc, char *argv[])
{
	...
	do
	{
		...
		case monster_wins:
			printf("%s has defeated %s. What an evil git.\n", monster_type, player_name);
			current = end;
			break;
