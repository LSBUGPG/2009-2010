...
void update(Image &table, GLfloat t)
{
}
...
void draw(Ball &ball, const GLfloat *position)
{
	draw(ball.image, position);
}

void draw(Paddle &paddle, const GLfloat *position)
{
	draw_score(paddle.score, position);
}
...
int main(int argc, char *argv[])
{
	...
	while (true) 
	{
		...
		draw(ball, ball.position);
		draw(left, left_pos);
		draw(right, right_pos);
		end_render();
    }
