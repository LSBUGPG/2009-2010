...
bool init_graphics(void)
{
	...
	font = TTF_OpenFont("Primitive.ttf", 10);//changed writing font
        ...
        if (font)
        {
                ...
                if (surface)
                {
                        if (surface->format->BitsPerPixel == 32)
                        {
                                ...
                                image.dimension[0] = GLfloat(surface->w) / 2.0f;
                                image.dimension[1] = GLfloat(surface->h) / 2.0f;
                        }
