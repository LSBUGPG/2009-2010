...
struct Laser: public Object
{
	...
	void update(void)
	{
		if (alive)
		{
			...
			if (move_up && position[0] > PLAY_FIELD_LEFT + LASER_WIDTH)
			{
				position[1]++;
			}
			
			if (move_down && position[0] < PLAY_FIELD_RIGHT - LASER_WIDTH)
			{
				position[1]--;
			}
...
{
	char filename[] = "mecha?.bmp";///changed the image of the ship
	for (int i = 0; i < LASER_FRAMES; i++)
	{
		sprintf_s(filename, sizeof(filename), "mecha%d.bmp", i);
...
{
	laser.lives = 4;//changed amount of lives
	shot.alive = false;
	fire_laser = false;
	move_left = false;
	move_right = false;
